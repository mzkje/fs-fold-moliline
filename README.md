...
- 基础：注册/路由、请求应答、事件广播、命令执行；
- 可靠：WAL 日志、同一请求只执行一次、死信、重试、自动重连、重启补发；
- 多节点：心跳选主、断主约 6 秒交接、防脑裂；
- 扩展（模型世界，当前示例可跑）：把模型折成 .fs 世界，经总线启动/停止，
  一个模型可“进入”另一个世界读写文件并保存；
- 历史实验记录（复现脚本未全部随仓库打包）：知识/词条在程序间真转移、
  向黑盒程序注入输入并读回、画面定位后点击、HTTP 承载与网页浏览器原型、
  高吞吐/并联加速曲线、折叠世界驱动真实程序。

详细清单与“可直接跑 vs 历史记录”的区分见 `02-moliline/README.md`。
