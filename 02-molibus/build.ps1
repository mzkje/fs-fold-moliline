# build.ps1 - compile MoliBus v1.0 baseline with Windows built-in csc
...
$targets = @{
  "moli_bus_tcp.cs" = "moli_bus_tcp.exe";
...
$outBus = Join-Path $bin "moli_bus_refactor.exe"
& $csc /nologo /out:$outBus (Join-Path $rf "MoliOptions.cs") (Join-Path $rf "WalStore.cs") (Join-Path $rf "BusRegistry.cs") (Join-Path $rf "LeaderElection.cs") (Join-Path $src "MoliWire.cs") (Join-Path $rf "MoliBusRefactor.cs") | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Error "refactor build failed moli_bus_refactor"; exit 1 }
