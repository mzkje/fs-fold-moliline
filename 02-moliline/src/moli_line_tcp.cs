// MoliLine over TCP: central hub with per-client lines.
...
class MoliLineTcp
{
    ...
    static string Dead = Path.Combine(Path.GetTempPath(),
        "moli_line_dead");
    static string WalDir = Path.Combine(Path.GetTempPath(), "moli_line_wal");
    ...
    static int Main()
    {
        ...
        listener.Start();
        Log("MoliLine TCP bus " + port + " nodes=" + NodeN +
